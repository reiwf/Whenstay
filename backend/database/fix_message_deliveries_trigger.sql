-- Fix for message_deliveries_status_ts trigger function
-- This fixes the "case not found" error by adding an ELSE clause

CREATE OR REPLACE FUNCTION message_deliveries_status_ts()
RETURNS trigger
LANGUAGE plpgsql
AS $$
begin
  ...
  if tg_op = 'UPDATE' and coalesce(old.status,'') <> new.status then
    case new.status
      ...
      when 'read'      then if new.read_at      is null then new.read_at      := now(); end if;
        ...
        -- No timestamp update needed for failed status
        null;
    end case;
  end if;
