...
class CalendarService {
   ...
   * Handles unassigned reservations (room_unit_id = NULL, room_unit_number = 'UNASSIGNED')
   */
    ...
    timelineData.forEach(item => {
      ...
      if (!roomTypesMap.has(item.room_type_id)) {
        roomTypesMap.set(item.room_type_id, {
          ...
          units: new Map(),
          hasUnassigned: false // Track if this room type has unassigned reservations
        });
      }

      const roomType = roomTypesMap.get(item.room_type_id);
      
      // Handle unassigned reservations (room_unit_id is NULL)
      if (item.room_unit_id === null && item.room_unit_number === 'UNASSIGNED') {
        // Mark this room type as having unassigned reservations
        roomType.hasUnassigned = true;
        
        // Create or get the special "UNASSIGNED" unit for this room type
        const unassignedKey = `UNASSIGNED_${item.room_type_id}`;
        if (!roomType.units.has(unassignedKey)) {
          roomType.units.set(unassignedKey, {
            id: null, // Keep null to indicate unassigned
            number: 'UNASSIGNED',
            roomTypeId: item.room_type_id,
            isUnassigned: true
          });
        }
      } else if (item.room_unit_id !== null) {
        // Handle regular assigned room units
      ...
      const reservationData = {
        ...
        roomUnitId: item.room_unit_id, // This will be null for unassigned reservations
        roomTypeId: item.room_type_id,
    ...
    const roomTypes = Array.from(roomTypesMap.values())
      .map(roomType => ({
        ...
        units: Array.from(roomType.units.values())
          .sort((a, b) => {
            // Sort unassigned units to the bottom of each room type
            if (a.isUnassigned && !b.isUnassigned) return 1;
            if (!a.isUnassigned && b.isUnassigned) return -1;
            if (a.isUnassigned && b.isUnassigned) return 0;
            // Regular units sorted numerically
